...
class Analyzer {
  ...
  public static function get_tokens_value($file_name, $tokens, $var_declares, $var_declares_global, $tokenid, $start = 0, $stop = 0, $source_functions = []) {
    $value = '';
    if (!$stop) {
      $stop = count($tokens);
    }
      ...
      if (is_array($tokens[$i])) {
        ...
        if (T_VARIABLE === $tokens[$i][0]
          ...
          if (!in_array($tokens[$i][1], Sources::$V_USERINPUT)) {
            ...
            if (isset($tokens[$i][3])) {
              $parameter_name = str_replace(["'", '"'], '', $tokens[$i][3][0]);
            }
            else {
              $parameter_name = '';
            }
  ...
  public static function get_var_value($file_name, $var_token, $var_declares, $var_declares_global, $last_token_id, $source_functions = []) {
    $var_value = '';
  ...
  public static function getBraceEnd($tokens, $i) {
    $c = 1;
    $newbraceopen = 1;
    while (!(0 === $newbraceopen || ';' === $tokens[$i + $c])) {
  ...
  public static function get_ini_paths($path) {
