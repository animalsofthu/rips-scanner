  ...
  public function prepare_tokens() {
    ...
    for ($i = 0, $max = count($this->tokens); $i < $max; $i++) {
      if (is_array($this->tokens[$i])) {
        ...
        elseif (T_OPEN_TAG_WITH_ECHO === $this->tokens[$i][0]) {
    ...
    for ($i = 0; $i < ($max = count($this->tokens)); $i++) {
      ...
      elseif (is_array($this->tokens[$i])) {
        ...
        if ((T_IF === $this->tokens[$i][0] || T_ELSEIF === $this->tokens[$i][0] || T_FOR === $this->tokens[$i][0]
          ...
          elseif ('{' !== $this->tokens[$i + $f] && ';' !== $this->tokens[$i + $f]) {
            $c = 1;
        ...
        elseif (T_SWITCH === $this->tokens[$i][0] && '(' === $this->tokens[$i + 1]) {
          $newbraceopen = 1;
          $c = 2;
          while (0 !== $newbraceopen) {
            ...
            elseif (!isset($this->tokens[$i + $c]) || ';' === $this->tokens[$i + $c]) {
        ...
        elseif (T_DEFAULT === $this->tokens[$i][0]
          && '{' !== $this->tokens[$i + 2]) {
          $f = 2;
          $newbraceopen = 0;
        ...
        elseif (T_FUNCTION === $this->tokens[$i][0]) {
          $this->tokens[$i + 1][1] = strtolower($this->tokens[$i + 1][1]);
        }
        elseif (T_STRING === $this->tokens[$i][0] && '(' === $this->tokens[$i + 1]) {
        ...
        elseif (T_DO === $this->tokens[$i][0]) {
          ...
          while (T_WHILE !== $this->tokens[$i + $f][0] || $otherDOs) {
            ...
            elseif (T_WHILE === $this->tokens[$i + $f][0]) {
              $otherDOs--;
            }
            $f++;
  ...
  public function fix_ternary() {
    for ($i = 0, $max = count($this->tokens); $i < $max; $i++) {
      if ('?' === $this->tokens[$i]) {
        ...
        elseif (is_array($this->tokens[$i - 1]) && T_VARIABLE === $this->tokens[$i - 1][0]) {
          unset($this->tokens[$i - 1]);
        }
      }
    }
