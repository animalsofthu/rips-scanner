...
class Tokenizer {
  ...
  public function wrapbraces($start, $between, $end) {
    $this->tokens = array_merge(
      array_slice($this->tokens, 0, $start), ['{'],
      array_slice($this->tokens, $start, $between), ['}'],
    ...
    for ($i = 0, $max = count($this->tokens); $i < $max; $i++) {
      if (is_array($this->tokens[$i])) {
        if (in_array($this->tokens[$i][0], Tokens::$T_IGNORE)) {
          ...
          if (T_CLOSE_TAG === $this->tokens[$i][0]) {
            $this->tokens[$i] = ';';
          }
          else {
            if (T_OPEN_TAG_WITH_ECHO === $this->tokens[$i][0]) {
      ...
      // @ (depress errors) disturbs connected token handling
        ...
        // rewrite $array{index} to $array[index]
          ...
          if ('{' === $this->tokens[$i]
            && isset($this->tokens[$i - 1]) && ((is_array($this->tokens[$i - 1]) && T_VARIABLE === $this->tokens[$i - 1][0])
              || ']' === $this->tokens[$i - 1])) {
            $this->tokens[$i] = '[';
            $f = 1;
  ...
  public function fix_tokens() {
    for ($i = 0; $i < ($max = count($this->tokens)); $i++) {
      ...
      if ('`' === $this->tokens[$i]) {
        ...
        if (!empty($line_nr)) {
          $this->tokens[$i + $f] = ')';
          $this->tokens[$i] = [T_STRING, 'backticks', $line_nr];
      ...
      // real token
        ...
        if (is_array($this->tokens[$i])) {
          ...
          if ((T_IF === $this->tokens[$i][0] || T_ELSEIF === $this->tokens[$i][0] || T_FOR === $this->tokens[$i][0]
              || T_FOREACH === $this->tokens[$i][0] || T_WHILE === $this->tokens[$i][0]) && '(' === $this->tokens[$i + 1]) {
            // skip condition in ( )
            $f = 2;
            $braceopen = 1;
            while (0 !== $braceopen) {
            ...
            // if body not in { (and not a do ... while();) wrap next instruction in braces
              ...
              if ('{' !== $this->tokens[$i + $f] && ';' !== $this->tokens[$i + $f]) {
                $c = 1;
          ...
          // rebuild else without { }
            ...
            if (T_ELSE === $this->tokens[$i][0]
              && T_IF !== $this->tokens[$i + 1][0]
              && '{' !== $this->tokens[$i + 1]) {
              $f = 2;
            ...
            // rebuild switch (): endswitch;
              ...
              if (T_SWITCH === $this->tokens[$i][0] && '(' === $this->tokens[$i + 1]) {
                $newbraceopen = 1;
                $c = 2;
                while (0 !== $newbraceopen) {
                  ...
                  if ('(' === $this->tokens[$i + $c]) {
                    ...
                    if (')' === $this->tokens[$i + $c]) {
                      ...
                      if (!isset($this->tokens[$i + $c]) || ';' === $this->tokens[$i + $c]) {
                ...
                if (':' === $this->tokens[$i + $c]) {
                  $f = 1;
                  while (T_ENDSWITCH !== $this->tokens[$i + $c + $f][0]) {
                    $f++;
              ...
              // rebuild switch case: without { }
                ...
                if (T_CASE === $this->tokens[$i][0]) {
                  ...
                  if ((':' === $this->tokens[$i + $e] || ';' === $this->tokens[$i + $e])
                    && '{' !== $this->tokens[$i + $f]
                    && T_CASE !== $this->tokens[$i + $f][0] && T_DEFAULT !== $this->tokens[$i + $f][0]) {
                    $newbraceopen = 0;
                    while ($newbraceopen || (isset($this->tokens[$i + $f]) && '}' !== $this->tokens[$i + $f]
                        && !(is_array($this->tokens[$i + $f])
                          && (T_BREAK === $this->tokens[$i + $f][0] || T_CASE === $this->tokens[$i + $f][0]
                            || T_DEFAULT === $this->tokens[$i + $f][0] || T_ENDSWITCH === $this->tokens[$i + $f][0])))) {
                      if ('{' === $this->tokens[$i + $f]) {
                        $newbraceopen++;
                      }
                      else {
                ...
                // rebuild switch default: without { }
                  ...
                  // lowercase all function names because PHP doesn't care
                    ...
                    if (T_FUNCTION === $this->tokens[$i][0]) {
                      $this->tokens[$i + 1][1] = strtolower($this->tokens[$i + 1][1]);
                    }
                    else {
                      if (T_STRING === $this->tokens[$i][0] && '(' === $this->tokens[$i + 1]) {
                      ...
                      // and we need the condition to be parsed before the loop tokens)
                        ...
                        if (T_DO === $this->tokens[$i][0]) {
                          // don't rewrite where do() is a class method
                          if (T_FUNCTION === ($previous = $this->tokens[$i - 1][0])
                            || T_PAAMAYIM_NEKUDOTAYIM === $previous
                            || T_OBJECT_OPERATOR === $previous) {
                            break;
                          }
                          ...
                          while (T_WHILE !== $this->tokens[$i + $f][0] || $otherDOs) {
                            if (T_DO === $this->tokens[$i + $f][0]) {
                              $otherDOs++;
                            }
                            else {
                              if (T_WHILE === $this->tokens[$i + $f][0]) {
                                $otherDOs--;
                              }
                            }
                            $f++;
  ...
  public function array_reconstruct_tokens() {
    for ($i = 0, $max = count($this->tokens); $i < $max; $i++) {
      // check for arrays
      if (is_array($this->tokens[$i]) && T_VARIABLE === $this->tokens[$i][0] && '[' === $this->tokens[$i + 1]) {
        $this->tokens[$i][3] = [];
        $has_more_keys = TRUE;
        $index = -1;
        $c = 2;
        ...
        while ($has_more_keys && $index < MAX_ARRAY_KEYS) {
          ...
          if ((T_CONSTANT_ENCAPSED_STRING === $this->tokens[$i + $c][0] || T_LNUMBER === $this->tokens[$i + $c][0] || T_NUM_STRING === $this->tokens[$i + $c][0] || T_STRING === $this->tokens[$i + $c][0]) && ']' === $this->tokens[$i + $c + 1]) {
            unset($this->tokens[$i + $c - 1]);
            $this->tokens[$i][3][$index] = str_replace([
              '"',
              "'",
            ], '', $this->tokens[$i + $c][1]);
    ...
    for ($i = 0, $max = count($this->tokens); $i < $max; $i++) {
      if ('?' === $this->tokens[$i]) {
        ...
        if (')' === $this->tokens[$i - 1]) {
          ...
          $newbraceopen = 1;
          $f = 2;
          while (0 !== $newbraceopen && ';' !== $this->tokens[$i - $f]) {
            if ('(' === $this->tokens[$i - $f]) {
              $newbraceopen--;
            }
            else {
          ...
          if ('!' === $this->tokens[$i - $f] || (is_array($this->tokens[$i - $f])
              && (T_STRING === $this->tokens[$i - $f][0] || T_EMPTY === $this->tokens[$i - $f][0] || T_ISSET === $this->tokens[$i - $f][0]))) {
            unset($this->tokens[$i - $f]);
          }
        ...
        // condition is a check or assignment
          ...
          if (in_array($this->tokens[$i - 2][0], Tokens::$T_ASSIGNMENT) || in_array($this->tokens[$i - 2][0], Tokens::$T_OPERATOR)) {
          ...
          // condition is a single variable, delete
            ...
            if (is_array($this->tokens[$i - 1]) && T_VARIABLE === $this->tokens[$i - 1][0]) {
              unset($this->tokens[$i - 1]);
            }
          }
        }
