...
if (!empty($_POST['loc'])) {
  ...
  if (file_exists($mbb_dir) && is_dir($mbb_dir)) {
    $builds = scandir($mbb_dir);
    sort($builds, SORT_NUMERIC);
    array_pop($builds);

    unset($builds[array_search('.', $builds)], $builds[array_search('..', $builds)]);

    $SKIPDIRS = array_merge($SKIPDIRS, $builds);
  }
  ...
  if (empty($_POST['search'])) {
    ...
    if (defined('MODE_CLI')) {
      echo "\n<div id=\"scan_metadata\" style=\"display: none;\">";
    }    //hide metadata
...
<div id="stats" class="stats">
  <table class="textcolor" width="100%">
    <tr>
      ...
      <th align="right">
        <input class="button" type="button" value="x" onClick="document.getElementById('stats').style.display='none';" title="close"/>
      </th>
    </tr>
  </table>
  <hr/>
  <table class="textcolor" width="100%">
    ...
    if (empty($_POST['search'])) {
      ...
      echo '<tr><td colspan="2">', ((0 == $count_matches) ? 'No' : $count_matches), ' matches found.</td></tr>';
    }
    ...
    <table class="textcolor" width="100%">
      <tr>
        <td nowrap width="160">Scan time:</td>
        <td nowrap>
          <span id="scantime"><?php printf('%.03f seconds', $elapsed); ?></span>
        </td>
      </tr>
    </table>
