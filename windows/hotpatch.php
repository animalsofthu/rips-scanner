...
<div style="padding: 20px; width: 400px">
  ...
  <div id="hotpatchbuild">
            ...
            <form id="<?php echo $name; ?>">
              <table class="textcolor" cellspacing=0px cellpadding=2px>
                ...
                foreach ($params as $param) {
                  if ('$_SERVER' != $name || ('$_SERVER' == $name && 'HTTP' == substr($param, 0, 4))) {
    ...
    if (isset($_GET['get'])) {
      creatediv($_GET['get'], '$_GET');
    }
    if (isset($_GET['post'])) {
      creatediv($_GET['post'], '$_POST');
    }
    if (isset($_GET['cookie'])) {
      creatediv($_GET['cookie'], '$_COOKIE');
    }
    if (isset($_GET['files'])) {
      creatediv($_GET['files'], '$_FILES');
    }
